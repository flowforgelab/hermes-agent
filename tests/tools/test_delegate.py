...
class TestDelegateHeartbeat(unittest.TestCase):
    ...
    def test_heartbeat_does_not_trip_idle_stale_while_inside_tool(self):
        ...
        def slow_run(**kwargs):
            # Long enough to exceed a deliberately tiny idle threshold, but
            # shorter than the patched in-tool threshold.
            time.sleep(0.25)
            return {"final_response": "done", "completed": True, "api_calls": 1}

        child.run_conversation.side_effect = slow_run

        # Patch interval + stale ceilings so the test proves threshold
        # selection, not wall-clock luck. If current_tool were ignored, the
        # idle ceiling of 1 stale cycle would stop after the first repeat.
        # With current_tool set, the in-tool ceiling keeps heartbeats alive.
        with (
        ...
        self.assertGreater(
            ...
            f"got {len(touch_calls)} touches over 0.25s at 0.02s interval",
        )
